...
Plane::Plane(Double a, Double b, Double c, Double d)
    ...
    , d(d)
{
    if (!Vector3(a, b, c).length().close(1))
    {
...
Plane::Plane(const Vector3& normal, const Vector3& point)
    ...
    , d(dot(normal, point))
{
    if (!normal.length().close(1)) { throw std::runtime_error("Normal must be normalized!"); }
}

Vector3 Plane::GetPointClosestToCentre() const { return GetNormal() * d; }
...
std::tuple<IntersectionType, std::vector<Vector3>> IntersectOnEarth(const Plane& a, const Plane& b)
{
    ...
    std::vector<Double> sols =
        SolveQuadratic(line.dir.length2(), 2 * dot(line.dir, line.point), line.point.length2() - 1);
